        ...
        const string& rotationInterval)
{
    ...
    errorOutput->open(logDir + "/%F/errors-%F-%T.log.gz", rotationInterval);
    ...
    strategyOutput->logTo(
            "",
            logDir + "/%F/$(9)/$(10)/$(0)-%T.log.gz",
            createMatchedWinFile);

    logger.addOutput(
            strategyOutput,
            boost::regex("MATCHEDWIN|MATCHEDLOSS|MATCHEDIMPRESSION|MATCHEDCLICK"));
    //            boost::regex("MATCHEDWIN|MATCHEDLOSS|MATCHEDIMPRESSION|MATCHEDCLICK|AUCTION|BID"));
}
