...
tmux new-session -d -s rtb './build/x86_64/bin/launcher --node localhost --script ./launch_smaato_fixed.sh --launch rtbkit/ola.launch.json'
...
tmux new-window -d -t rtb:9 -n 'ola-fixed-price-agent' 'tail -F ./logs/ola-fixed-price-agent.log'
if [[ "$*" != *--quiet* ]]
then
    tmux attach -t rtb
fi
