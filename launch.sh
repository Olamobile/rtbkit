...
tmux new-session -d -s rtb './build/x86_64/bin/launcher --node localhost --script ./launch.sh --launch rtbkit/ola.launch.json'
...
tmux new-window -d -t rtb:9 -n 'ola-bidding-agent' 'tail -F ./logs/ola-bidding-agent.log'
if [[ "$*" != *--quiet* ]]
then
    tmux attach -t rtb
fi
